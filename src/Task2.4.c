...
run_time = 20.0; // Set the initial run time to 3 seconds.
...
plot(ref,theta, Fs, no_of_samples , PS, "Motor Position vs Time", "Time(s)", "Motor Position(Degrees)"); 
break;

case 'i':
			printf("Enter a new Ti:\n");
			scanf("%f", &Ti);
			printf("Kp = %f Ti = %f Td = %f N = %d Fs = %f run_time = %f\n", Kp,Ti,Td,N,Fs,run_time);
			break;
